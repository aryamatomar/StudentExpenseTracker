  ...
  if (active && payload && payload.length) {
    return (
      <div className="bg-slate-900/95 text-white p-3 rounded-xl shadow-xl border border-slate-700 text-xs backdrop-blur-md">
        ...
        <p className="text-brand-300 font-extrabold text-sm">{formatCurrency(payload[0].value)}</p>
    ...
    <div className="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-soft flex flex-col justify-between">
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-xs text-slate-500">Expenditure flow over past months</p>
        </div>
      ...
      <div className="pt-3 border-t border-slate-100 flex items-center justify-between text-xs text-slate-500">
        <span>💡 Keep an eye on peak exam and semester start months!</span>
      </div>
    </div>
  );
};
